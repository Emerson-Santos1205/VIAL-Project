...
A Decision is an organizational determination that establishes what should happen or what should be maintained.

A Decision determines intent; it does not by itself grant Authorization, grant Approval, or execute.
...
Authority is the organizational basis by which Authorization is granted for a specific class of Decisions within a defined scope.
...
An Operational Decision determines an action within a defined operational domain.

The action becomes authorized only through the authority model, and requires Approval when policy demands it.
...
When a consequential Decision depends on a stabilized evaluation boundary, the referenced Context SHOULD be FROZEN. After FROZEN, the normative content of that Context MUST NOT change; later changes require a new Context rather than mutation of the existing one.
...
Escalation is an event or transition, not a Decision state.
...
Approval is an explicit additional step within the Authorization workflow; it does not replace Authorization.
...
Where Approval is required, the Decision becomes eligible to proceed only when the required approval conditions are satisfied within the applicable Authorization workflow.
...
ESCALATION is not a lifecycle state. It is an event or transition that routes a Decision toward additional authority or human review.
...
CANCELLED, FAILED and REVOKED are terminal states reachable from applicable states. ESCALATION is an event, not a state, and does not appear in the state machine.
